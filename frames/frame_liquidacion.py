...
class FrameLiquidacion(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        # Usar DateSelector para mostrar un calendario
        self.date_picker = DateSelector(fecha_frame, 
                                   width=14, 
        ...
        self.date_picker.bind('<<DateSelected>>', self.on_fecha_cambio)
        # debug removido
        ...
        # Botón de mover fecha liquidación
        self.btn_mover_fecha = ttk.Button(base_frame, text="📅 Mover Fecha Liquidación",
                                          style='Blue.TButton',
                                          command=self.mover_fecha_liquidacion)
        self.btn_mover_fecha.pack(fill='x', pady=6)
    ...
    def actualizar_liquidacion(self):
        ...
        try:
            ...
            try:
                ...
                # Actualizar tarjetas que NO dieron (si existe el label)
                if 'tarjetas_sin_abono' in self.labels_estadisticas:
                    val = datos.get('tarjetas_sin_abono', 0)
                    self.labels_estadisticas['tarjetas_sin_abono'].config(text=f"{val}")
    ...
    def mostrar_tabla_tarjetas(self):
        """Método de compatibilidad para VentanaEditarTarjeta. Refresca la liquidación."""
        self.actualizar_liquidacion()
        # Refrescar ventanas de detalle abiertas
        self._refrescar_ventanas_detalle()

    def mostrar_detalle_estadistica(self, tipo_estadistica):
        """Muestra ventana con detalles desde la API según el tipo."""
        if not self.empleado_actual_id:
            messagebox.showwarning("Advertencia", "Seleccione un empleado")
            return
        fecha_str = self.fecha_actual.strftime('%Y-%m-%d')
        ...
        try:
            ...
            if tipo_estadistica == 'tarjetas_canceladas':
                titulo = "Tarjetas canceladas del día (Doble clic para ver abonos)"
                cols = ("Cliente", "Cancelado", "Monto", "Ruta")
                contexto = 'tarjetas_canceladas'
            elif tipo_estadistica == 'tarjetas_nuevas':
                titulo = "Tarjetas nuevas del día (Clic derecho para opciones)"
                cols = ("Cliente", "Teléfono", "Dirección", "Monto", "Fecha", "Interés", "Cuotas", "Ruta")
                contexto = 'tarjetas_nuevas'
            elif tipo_estadistica == 'tarjetas_sin_abono':
                titulo = "Tarjetas activas que NO dieron abono hoy"
                cols = ("Cliente", "Cuota", "Monto Préstamo", "Atraso")
                contexto = 'tarjetas_sin_abono'
            elif tipo_estadistica == 'total_registros':
                titulo = "Abonos del día (Clic derecho para opciones)"
                cols = ("Cliente", "Abonado", "Método", "Monto", "Fecha")
                contexto = 'abonos_dia'
            else:
                return

            # Verificar si ya existe la ventana para este contexto
            ventana = None
            tree = None
            iid_to_index = {}
            
            if contexto in self._detalles_abiertos:
                v = self._detalles_abiertos[contexto]
            ...
            # Si no se reutilizó, crear nueva
            if not ventana:
                ventana, tree, iid_to_index = self._crear_ventana_detalle_empty(titulo, cols, contexto)
                # Bindings
                if tipo_estadistica == 'tarjetas_canceladas':
                    tree.bind("<Double-1>", self._ver_abonos_tarjeta_cancelada)
                elif tipo_estadistica == 'tarjetas_nuevas':
                    tree.bind("<Button-3>", self._menu_contextual_nuevas)
                elif tipo_estadistica == 'total_registros':
                    tree.bind("<Button-3>", self._menu_contextual_abonos)
                ...
                try:
                    if tipo_estadistica == 'tarjetas_canceladas':
                        ...
                        for t in items:
                            ...
                            filas.append({
                                'iid': codigo_tarjeta,
                                'values': (cliente, f"$ {cancelado:,.0f}", f"$ {monto:,.0f}", ruta)
                            })
                        ...
                        for t in items:
                            ...
                            filas.append({
                                'iid': codigo_tarjeta,
                                'values': (cliente, telefono, direccion, f"$ {monto:,.0f}", fecha_txt, interes_txt, cuotas, ruta)
                            })

                    elif tipo_estadistica == 'tarjetas_sin_abono':
                        items = self.api_client.list_tarjetas_sin_abono_dia(self.empleado_actual_id, fecha_str, self.token_tz)
                        for t in items:
                            cliente = f"{t.get('cliente_apellido', '')} {t.get('cliente_nombre', '')}".strip().upper()
                            
                            monto = float(t.get('monto') or 0)
                            n_cuotas = int(t.get('cuotas') or 1)
                            interes = float(t.get('interes') or 0)
                            
                            # Calcular cuota real: Monto * (1 + Interes/100) / Cuotas
                            cuota_real = 0
                            if n_cuotas > 0:
                                total_con_interes = monto * (1 + interes/100.0)
                                cuota_real = total_con_interes / n_cuotas
                            
                            atraso = str(t.get('atraso') or 0)
                            codigo = t.get('codigo')
                            
                            filas.append({
                                'iid': codigo,
                                'values': (cliente, f"$ {cuota_real:,.0f}", f"$ {monto:,.0f}", atraso)
                            })
                        ...
                        for a in items:
                            ...
                            abono_id = str(a.get('id') or a.get('abono_id') or '')
                            filas.append({
                                'iid': abono_id,
                                'values': (cliente, f"$ {abonado:,.0f}", metodo_txt, f"$ {monto_prestamo:,.0f}" if monto_prestamo else "", fecha_txt)
                            })
                            
                except Exception as e:
                    logger.error(f"Error al cargar detalle de {tipo_estadistica}: {e}")
                    filas = [{'iid': 'err', 'values': ("Error al cargar",)}]
                finally:
    ...
    def _ver_abonos_tarjeta_cancelada(self, event):
        tree = event.widget
        sel = tree.selection()
        if not sel: return
        codigo_tarjeta = sel[0]
        # Filtrar IDs generados automáticamente o de error
        if not codigo_tarjeta or codigo_tarjeta.startswith('row-') or codigo_tarjeta == 'err': 
            return

        # Mostrar abonos de esa tarjeta en un popup
        try:
            # Fetch en hilo secundario idealmente, pero para popup simple hacemos síncrono rápido
            abonos = self.api_client.list_abonos_by_tarjeta(codigo_tarjeta)
            
            # Usar la ventana padre (listado de tarjetas) como padre del popup
            try:
            ...
            top.title(f"Abonos Tarjeta {codigo_tarjeta}")
            top.geometry("450x300")
            
            # Hacerla modal sobre la ventana padre para evitar que se pierda
            ...
            cols = ("Fecha", "Monto", "Método")
            t = ttk.Treeview(top, columns=cols, show='headings', height=10)
            t.heading("Fecha", text="Fecha")
            t.heading("Monto", text="Monto")
            t.heading("Método", text="Método")
            t.column("Fecha", width=100)
            t.column("Monto", width=100)
            t.column("Método", width=100)
            t.pack(fill='both', expand=True, padx=10, pady=10)
            
            if not abonos:
                t.insert('', 'end', values=("Sin abonos", "", ""))
            
            total = 0.0
            for a in abonos:
                fecha = self._parse_iso_date_only(a.get('fecha') or a.get('created_at'))
                monto = float(a.get('monto', 0) or 0)
                total += monto
                metodo = a.get('metodo_pago') or 'Efectivo'
                t.insert('', 'end', values=(fecha, f"${monto:,.0f}", metodo))
            
            lbl_tot = ttk.Label(top, text=f"Total Abonado: ${total:,.0f}", font=('Segoe UI', 10, 'bold'))
            lbl_tot.pack(pady=10)
                
        except Exception as e:
            messagebox.showerror("Error", f"No se pudieron cargar los abonos: {e}")

    def _menu_contextual_nuevas(self, event):
        tree = event.widget
        ...
        menu.add_command(label="✏️ Editar Tarjeta", command=lambda: self._editar_tarjeta_nueva_accion(iid))
        menu.add_command(label="❌ Eliminar Tarjeta", command=lambda: self._eliminar_tarjeta_nueva_accion(iid))
        menu.post(event.x_root, event.y_root)

    def _editar_tarjeta_nueva_accion(self, codigo_tarjeta):
        if codigo_tarjeta.startswith('row-') or codigo_tarjeta == 'err':
            messagebox.showwarning("Error", "No se puede editar este elemento.")
            return
        try:
            # Reusar VentanaEditarTarjeta.
            # Se asume que self tiene 'mostrar_tabla_tarjetas' (lo agregamos antes)
            VentanaEditarTarjeta(self, codigo_tarjeta)
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo abrir editor: {e}")

    def _eliminar_tarjeta_nueva_accion(self, codigo_tarjeta):
        if codigo_tarjeta.startswith('row-') or codigo_tarjeta == 'err': return
        
        if not messagebox.askyesno("Confirmar Eliminación", 
                                   "¿Está seguro de eliminar esta tarjeta?\n\n"
                                   "Se eliminará el registro del préstamo y todos sus abonos asociados.\n"
                                   "Esta acción afectará la caja del día si la tarjeta fue creada hoy."):
            return
        try:
            res = self.api_client.delete_tarjeta(codigo_tarjeta)
            if res:
                messagebox.showinfo("Éxito", "Tarjeta eliminada correctamente")
                self.mostrar_tabla_tarjetas() # Refresca todo
                # Recalcular caja
                try:
                    fecha_str = self.fecha_actual.strftime('%Y-%m-%d')
                    self.api_client.recalcular_caja_dia(self.empleado_actual_id, fecha_str)
                except: pass
            else:
                messagebox.showerror("Error", "No se pudo eliminar la tarjeta (API retornó False)")
        except Exception as e:
             messagebox.showerror("Error", f"Fallo al eliminar: {e}")

    def _menu_contextual_abonos(self, event):
        tree = event.widget
        iid = tree.identify_row(event.y)
        if not iid: return
        tree.selection_set(iid)
        ...
        menu = tk.Menu(self, tearoff=0)
        menu.add_command(label="✏️ Editar Abono", command=lambda: self._editar_abono_accion(iid))
        menu.add_command(label="❌ Eliminar Abono", command=lambda: self._eliminar_abono_accion(iid))
        menu.post(event.x_root, event.y_root)

    def _editar_abono_accion(self, abono_id):
        if str(abono_id).startswith('row-') or abono_id == 'err': return
        
        # Diálogo simple para editar monto
        ...
        ttk.Label(top, text="Nuevo Monto:", font=('Segoe UI', 10)).pack(pady=(20,5))
        e_monto = ttk.Entry(top, font=('Segoe UI', 11))
        e_monto.pack(pady=5)
        e_monto.focus()
        
        # Intentar cargar valor actual
        try:
            abono = self.api_client.get_abono(int(abono_id))
            val = int(float(abono.get('monto', 0)))
            e_monto.insert(0, str(val))
        except: pass

        def save(event=None):
            try:
                nm_str = e_monto.get().strip()
                if not nm_str: return
                nm = float(nm_str)
                if nm <= 0:
                    messagebox.showwarning("Error", "El monto debe ser mayor a 0")
                    return

                self.api_client.update_abono(int(abono_id), {'monto': nm})
                messagebox.showinfo("Éxito", "Abono actualizado")
                top.destroy()
                self.mostrar_tabla_tarjetas() # Refresca liquidación y detalles
                # Recalcular caja
                try:
                    fecha_str = self.fecha_actual.strftime('%Y-%m-%d')
                    self.api_client.recalcular_caja_dia(self.empleado_actual_id, fecha_str)
        ...
        ttk.Button(top, text="Guardar", command=save, style='Blue.TButton').pack(pady=20)
        e_monto.bind('<Return>', save)

    def _eliminar_abono_accion(self, abono_id):
        if str(abono_id).startswith('row-') or abono_id == 'err': return

        if not messagebox.askyesno("Confirmar", "¿Eliminar este abono permanentemente?"):
            return
        try:
            self.api_client.delete_abono(int(abono_id))
            messagebox.showinfo("Éxito", "Abono eliminado")
            self.mostrar_tabla_tarjetas() # Refresca
            # Recalcular caja
            try:
                fecha_str = self.fecha_actual.strftime('%Y-%m-%d')
                self.api_client.recalcular_caja_dia(self.empleado_actual_id, fecha_str)
    ...
    def _prerender_into_tree(self, ventana, tree, filas, iid_to_index):
        """Inserta filas en lotes dentro de un tree existente.
           Soporta filas como tuplas (legacy) o dicts {'iid':..., 'values':...}
        """
        batch_size = 200
        total = len(filas)
        t0_pr = _pc()
        def _insert_batch(start_idx: int):
            ...
            for i in range(start_idx, end_idx):
                ...
                # Detectar formato
                if isinstance(raw_fila, dict) and 'values' in raw_fila:
                    vals = raw_fila['values']
                    iid = raw_fila.get('iid', f"row-{i}")
                else:
                    vals = raw_fila
                    iid = f"row-{i}"
                ...
                try:
                    ...
                    # Si el iid ya existe (raro pero posible en recargas parciales), intentar otro
                    try:
                        alt_iid = f"{iid}_{i}"
                        tree.insert('', 'end', iid=alt_iid, values=vals, tags=(tag,))
                        iid_to_index[alt_iid] = i
                    except: pass
    ...
    def mostrar_detalle_recaudos_metodo(self, metodo: str):
        ...
        logger.info(f"Abriendo detalle recaudos para método: {metodo}")
        if not self.empleado_actual_id:
            messagebox.showwarning("Advertencia", "Seleccione un empleado")
            return
    ...
    def _parse_iso_date_only(self, fecha) -> str:
        """Convierte ISO o tipo datetime a 'dd/mm/YYYY' sin hora, ajustado a zona horaria."""
        if not fecha:
            return ""
        try:
            ...
            # Si es naive (sin zona horaria), asumimos que es UTC (común en respuestas de BD/API)
            if dt.tzinfo is None:
                dt = dt.replace(tzinfo=timezone.utc)
            
            # Obtener timezone del usuario desde el cliente API
            tz_name = self.api_client.get_user_timezone() or 'America/Bogota'
            try:
    ...
    def mover_fecha_liquidacion(self):
        """Mueve los datos de liquidación de la fecha actual a otra."""
        if not self.empleado_actual_id:
            messagebox.showwarning("Selección", "Seleccione un empleado primero.")
            return
        
        fecha_origen = self.fecha_actual
        if not fecha_origen:
            messagebox.showwarning("Selección", "Seleccione una fecha primero.")
            return

        # Diálogo para fecha destino
        from tkinter import simpledialog, Toplevel
        
        # Crear ventana modal
        top = Toplevel(self)
        top.title("Mover Liquidación")
        ...
        ttk.Label(top, text=f"Mover datos del {fecha_origen} a:", font=('Arial', 10, 'bold')).pack(pady=(20, 10))
        
        # Date picker for destination
        cal_dest = None
        entry_dest = None
        ...
        try:
            ...
            yesterday = fecha_origen - timedelta(days=1)
            cal_dest = DateSelector(top, width=12, date_pattern='yyyy-MM-dd')
            cal_dest.set_date(yesterday)
            cal_dest.pack(pady=10)
        except ImportError:
            # Fallback simple entry
            from datetime import timedelta
            entry_dest = ttk.Entry(top)
            entry_dest.pack(pady=10)
            entry_dest.insert(0, str(fecha_origen - timedelta(days=1)))
            ttk.Label(top, text="(Formato: YYYY-MM-DD)", font=('Arial', 8)).pack()

        def confirmar():
            try:
                if cal_dest:
                    fecha_destino = cal_dest.get_date()
                else:
                    fecha_str = entry_dest.get()
                    from datetime import datetime
                    fecha_destino = datetime.strptime(fecha_str, '%Y-%m-%d').date()
    ...
    def _refrescar_ventanas_detalle(self):
        """Refresca el contenido de las ventanas de detalle abiertas sin cerrarlas."""
        try:
            # Iterar sobre una copia de los items para evitar problemas si el diccionario cambia
            for ctx, ventana in list(self._detalles_abiertos.items()):
                if not ventana or not ventana.winfo_exists():
                    # Limpiar referencias muertas
                    del self._detalles_abiertos[ctx]
                    continue
                
                # Relanzar la carga de datos según el contexto
                if ctx == 'abonos_dia':
            ...
            logger.error(f"Error refrescando detalles: {e}")
